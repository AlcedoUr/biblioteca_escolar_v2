...
// 2. SEGURIDAD DE NAVEGACIÓN (Bloqueo de Vistas por Rol)
...
// Si es DOCENTE, restringimos acceso a reportes administrativos, usuarios, etc.
// Lista blanca de páginas permitidas para docentes:
$paginas_docente = ['catalogo.php', 'biblioteca_virtual.php', 'reservas.php'];
if ($rol == 'DOCENTE' && !in_array($pagina_actual, $paginas_docente)) {
    header('Location: catalogo.php'); // Redirigir a su home si intenta entrar a dashboard
    exit;
}
?>
<!DOCTYPE html>
...
<div id="sidebar">
        ...
        <div class="d-flex align-items-center gap-3 p-3 rounded" style="background: rgba(0,0,0,0.2);">
            ...
            <div>
                <div class="small fw-bold text-white text-truncate" style="max-width: 120px;">
                    Hola, <?php echo explode(' ', $_SESSION['user_nombre'] ?? 'Usuario')[0]; ?>
                </div>
    ...
    <nav class="nav flex-column sidebar-menu">
        ...
        <?php if ($rol == 'ESTUDIANTE'): ?>
            <div class="text-white-50 small fw-bold mt-3 mb-1 ps-3">MI ESPACIO</div>
...
<div id="content">
    <div class="topbar">
        ...
        <div class="d-flex align-items-center gap-3">
            <div class="text-end text-muted lh-1 d-none d-sm-block" id="reloj-contenedor">
                <div class="fw-bold fs-5" id="reloj-hora">--:--:--</div>
                <small style="font-size: 0.75rem;" id="reloj-fecha">...</small>
            </div>
    ...
    <script>
        function actualizarReloj() {
            ...
            const h = ahora.toLocaleTimeString('es-PE', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true });
            const f = ahora.toLocaleDateString('es-PE', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
            document.getElementById('reloj-hora').innerText = h;
            document.getElementById('reloj-fecha').innerText = f;
        }
